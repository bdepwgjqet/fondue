 ...
 *       Compiler:  gcc
 *         Author:  bdep__  
 *        Created:  03/18/2012 02:31:54 PM
 ***/
...
struct line{ double x,y1,y2; int t; }ln[N];
double coor[N];
int top,ctop;
...
inline double length(int l,int r){ return coor[r-1]-coor[l-1]; }
int cmp(line a,line b){ return a.x<b.x; }

struct seg_tree
{
	int l,r,col,cnt,bl,br; double len;
}segt[N<<2];
...
double seg_len(int id,int ll,int rr)
{
	int l=segt[id].l,r=segt[id].r,m=(l+r)>>1;
	double ret=0;
...
int main()
{
	int n,i,j,l,r,tag,ca=1;
	double x1,x2,y1,y2,pre,ret;
	while(scanf("%d",&n)&&n)
	{
		printf("Test case #%d\n",ca++);
